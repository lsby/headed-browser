...
module.exports = async function (api, 网页窗口) {
    var { getHtml, runJs, sleep, click, alert, setProxy } = api

    // await setProxy('socks5://127.0.0.1:7890')
    ...
    fs.writeFileSync(
        path.resolve(__dirname, './猫猫.txt'),
        r.map((a) => cheerio.load(a)('#currentImg').attr('src')).join('\n') + '\n',
    )
